...
function updateCheck() {
    let price = 0;
    let dish = document.getElementsByName('cardDish');

    let tableCheck = document.getElementById('tableCheck');

    while (tableCheck.firstChild)
        tableCheck.removeChild(tableCheck.firstChild);

    let tabletemp = '';
    let tolpa = document.getElementById('defaultCheck4');
    dish.forEach(el => {
        ...
        if (inputs[0].name === 'count') {
            count = inputs[0].value;
            priceDish = inputs[1].value;
            name = inputs[1].name;
        } else {
            count = inputs[1].value;
            priceDish = inputs[0].value;
            name = inputs[0].name;
        }
        ...
        if (count > 0) {
            ...
            if (!tolpa.checked) {
                price += priceDish * count;
                tabletemp += `<tr>
                        <th>${name}</th>
                        <th>${count}</th>
                        <th>${priceDish * count}</th>
                    </tr>`
            } else {
                price += priceDish * count * 2.5;
                tabletemp += `<tr>
                        <th>${name}</th>
                        <th>${count * 5}</th>
                        <th><del>${priceDish * count * 5}</del>${priceDish * count * 2.5}</th>
                    </tr>`
            }
        }
    ...
    if (document.getElementById('defaultCheck2').checked) {
        price *= 1.2;
        strOptions += `<li>Быстрая доставка</li>`
    }
...
function plus(input) {
    let element = document.getElementById('count' + input);
